-- Update clone_campaign function to support cloning products without text options
CREATE OR REPLACE FUNCTION public.clone_campaign(
  p_campaign_id uuid, 
  p_clone_products boolean DEFAULT true,
  p_clone_text_options boolean DEFAULT true
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_clone_products THEN
    ...
    LOOP
      ...
      -- Clone text options only if requested
      IF p_clone_text_options THEN
